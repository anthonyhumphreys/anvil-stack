...
export function registerLifecycleHandlers(): void {
  ...
  ipcMain.handle('lifecycle:list-stages', (_event, workspaceId: string) => {
    try {
      return listLifecycleStages(workspaceId);
    } catch (err) {
      console.error('[Lifecycle IPC] Error listing stages:', err);
      throw err;
    }
  });

  ipcMain.handle(
    'lifecycle:update-stages',
    (_event, workspaceId: string, stages: LifecycleStageUpdate[]) => {
      try {
        return updateLifecycleStages(workspaceId, stages);
      } catch (err) {
        console.error('[Lifecycle IPC] Error updating stages:', err);
        throw err;
      }
    },
  );

  ipcMain.handle('lifecycle:reset-stages', (_event, workspaceId: string) => {
    try {
      return resetLifecycleStages(workspaceId);
    } catch (err) {
      console.error('[Lifecycle IPC] Error resetting stages:', err);
      throw err;
    }
  });
