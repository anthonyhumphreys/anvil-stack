 ...
 *
 * Provider coverage: AWS Lambda MicroVMs is implemented (ENV-03);
 * Cloudflare and Vercel land behind the same interface in ENV-04/ENV-05.
 */
...
function providerFor(
  ...
  switch (provider) {
    ...
    // ENV-04/ENV-05 land behind this same interface: Vercel via
    // @vercel/sandbox, Cloudflare via the account's provisioner Worker.
    case 'cloudflare-sandbox':
    case 'vercel-sandbox':
    case 'anvil-managed':
      throw new Error(`Provider ${provider} is not implemented on this build.`);
  }
}
