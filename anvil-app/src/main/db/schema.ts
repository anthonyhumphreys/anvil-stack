...
export const MIGRATIONS: Record<number, string> = {
  ...
  29: `
    ...
    SET label = '', criteria = '[]'
    WHERE gate = 'gate_1'
      AND label = 'Proceed to Ideation'
      AND json_array_length(criteria) = 2
      AND criteria LIKE '%Business Vision document%'
      AND criteria LIKE '%Steering approval%';

    UPDATE gate_templates
    SET label = '', criteria = '[]'
    WHERE gate = 'gate_2'
      AND label = 'Proceed to Discovery & Design'
      AND json_array_length(criteria) = 4
      AND criteria LIKE '%Impact analysis completed%'
      AND criteria LIKE '%Architecture Decision Records%'
      AND criteria LIKE '%Architecture review approval%'
      AND criteria LIKE '%Change classification assigned%';

    UPDATE gate_templates
    SET label = '', criteria = '[]'
    WHERE gate = 'gate_3'
      AND label = 'Proceed to Build'
      AND json_array_length(criteria) = 8
      AND criteria LIKE '%Security audit%'
      AND criteria LIKE '%Architecture diagram present%'
      AND criteria LIKE '%ADRs documenting key decisions%'
      AND criteria LIKE '%Architecture board approval%'
      AND criteria LIKE '%Technical design approval%'
      AND criteria LIKE '%Code review%'
      AND criteria LIKE '%DPIA generated%';

    UPDATE gate_templates
    SET label = '', criteria = '[]'
    WHERE gate = 'gate_4'
      AND label = 'Proceed to Run / Go-Live'
      AND json_array_length(criteria) = 9
      AND criteria LIKE '%Security audit%'
      AND criteria LIKE '%Code review%'
      AND criteria LIKE '%Compliance documents (DPIA)%'
      AND criteria LIKE '%Support documentation on Confluence%'
      AND criteria LIKE '%Handover pack generated%'
      AND criteria LIKE '%Steering approval%'
      AND criteria LIKE '%Change approval%'
      AND criteria LIKE '%Technical design approval%'
      AND criteria LIKE '%Architecture decisions up to date%';
  `,
  30: `
