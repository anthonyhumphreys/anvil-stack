...
export function MeshExecutionsPanel({
  localWorkerIncarnation,
}: {
  localWorkerIncarnation: string | null;
}): ReactNode {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    setError(null);
    try {
      const [jobsNext, handoffsNext, devicesNext] = await Promise.all([
      ...
      if (expandedJobId !== null) {
        try {
          ...
          // The expanded job may have been deleted server-side — collapse it
          // rather than erroring every poll on a row that no longer exists.
  ...
  // Drop the watch when its attempt is no longer live inside the expanded
  // job — otherwise a poll flipping it terminal (or collapsing the row)
  // would leave a renewing subscription pushing into a hidden pane.
  useEffect(() => {
    if (observingAttemptId === null) return;
    if (expandedJobId === null) {
      setObservingAttemptId(null);
      return;
    }
    const watched = (attemptsByJob[expandedJobId] ?? []).find((a) => a.id === observingAttemptId);
    if (watched === undefined || ATTEMPT_TERMINAL.has(watched.state)) {
      setObservingAttemptId(null);
    }
  }, [observingAttemptId, expandedJobId, attemptsByJob]);
  ...
  const canDecide = (approval: ApprovalRecord): boolean => {
    if (approval.approverEnrollmentId !== undefined) {
      return approval.approverEnrollmentId === selfEnrollmentId;
    }
    ...
    <div className="space-y-2">
      ...
      {jobs === null ? (
        ...
        <>
              ...
              <li key={job.id} className="rounded-md border border-border">
                ...
                {expandedJobId === job.id && (
                  <div className="space-y-3 border-t border-border-subtle px-3 py-2.5">
                    {job.stateReason !== undefined && (
                      <p className="text-xs text-text-tertiary">Reason: {job.stateReason}</p>
                    )}
                            ...
                            {approval.state === 'pending' &&
                              (canDecide(approval) ? (
                                ...
                                <span className="shrink-0 text-xs text-text-tertiary">
                                  {approval.approverEnrollmentId !== undefined
                                    ? `waiting on ${deviceName(approval.approverEnrollmentId)}`
                                    : "requested by this device's worker"}
                                </span>
                              ))}
                          </li>
                        ))}
          ...
          {jobs.length >= JOB_LIST_LIMIT && (
            <p className="text-xs text-text-tertiary">
              Showing the {JOB_LIST_LIMIT} most recent executions.
            </p>
          )}
        </>
      )}
