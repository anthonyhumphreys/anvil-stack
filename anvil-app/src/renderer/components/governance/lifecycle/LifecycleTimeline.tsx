...
const STAGES: Array<{ key: LifecycleStage; label: string; gate: GateId }> = [
  { key: 'ideation', label: 'Ideation', gate: 'gate_1' },
  { key: 'discovery_design', label: 'Discovery & Design', gate: 'gate_2' },
  { key: 'build', label: 'Build', gate: 'gate_3' },
  { key: 'run', label: 'Run', gate: 'gate_4' },
];
...
export function LifecycleTimeline({ item }: Props) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [dec, ia, hp, gateTemplates] = await Promise.all([
  ...
  const currentStageIdx = STAGE_INDEX[item.stage];
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-between">
          {STAGES.map((stage, idx) => {
            ...
            const gateReadiness = readiness[stage.gate];
            const gateDecision = latestDecisionPerGate[stage.gate];
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {STAGES.map((stage) => {
            ...
            return (
              <div key={stage.gate} className="rounded-lg border border-border bg-bg-tertiary p-3">
