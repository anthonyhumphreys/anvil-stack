...
import type { LifecycleItem, LifecycleStageDefinition } from '../../../../shared/types';
...
import { stageDotClass, stageLabel } from './stage-utils';
...
const TAB_CONFIG: Array<{
  ...
  { key: 'handover', label: 'Handover Pack', icon: PackageCheck },
];
...
export function LifecycleDetailView({ itemId }: Props) {
  ...
  useEffect(() => {
    ...
    window.anvil.lifecycle
      .listStages(item.workspaceId)
      .then(setStages)
      .catch((err) => {
  ...
  // If current tab is hidden (e.g. handover on an early stage), fall back
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="shrink-0 border-b border-border bg-bg-secondary px-5 py-3">
        <div className="flex items-center gap-3">
          <div className={`h-2.5 w-2.5 rounded-full ${stageDotClass(item.stage, stages)}`} />
