...
| --- | --- | --- |
| **Trusted Mesh Device** (desktop/daemon, pairing- or SAS-approved) | Full sync, job source, key delivery, dashboard grant issuer | ADK bundle (all held versions), own X25519 identity, task keys it sourced or received |
...
- `trusted` — approved by the user (SAS compare in Sync & Mesh settings) or redeemed pairing. All ADK delivery paths require this state.
