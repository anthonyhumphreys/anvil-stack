...
This repo is the first public alpha of Anvil. Describe what is implemented, what has a verification path, and what should be piloted before broad rollout. Do not describe the project as mature, production-hardened, or complete unless the code and docs genuinely support that claim.
...
Documentation must be operational and concrete: commands, endpoints, environment variables, policy decisions, report names, failure modes, and validation steps. Avoid vague launch copy and unsupported security guarantees.
