...
`POST /-/anvil/analyze` and `POST /-/anvil/llm-review` accept either one target (`packageName`, optional `version`) or a `targets` array. Target request bodies are validated, trimmed, deduplicated, and rejected with HTTP 400 when malformed rather than being allowed into queueing code.
