...
## Recommended v0 deployment model
...
The v0 adapter exposes a tested HTTP bridge for Lambda Function URL/API Gateway
...
For v0, prefer dedicated resources per Cell/environment where feasible. If shared physical resources are used, scope IAM to app-specific partition keys/prefixes where possible.
...
Open decision: one source of truth for v0.
...
verification before the preview adapter should be treated as production-ready.
...
The Cell author must not author SST/CDK directly in v0. The deployment adapter can use either tool internally.
