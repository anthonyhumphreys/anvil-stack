# Anvil Cloud v0 Implementation Spec

## Summary

Anvil Cloud v0 proves a small, agent-friendly, provider-neutral application model before adding broad deployment features.

The v0 product is a local-first TypeScript platform where a developer or coding agent can create an **Anvil Cell**, run it locally, inspect its state, and build deployable artefacts. Deployment adapters are included as later v0 milestones, but only after the local runtime and builder are stable. AWS is the first planned adapter.
...
- No Kubernetes, ECS, or container runtime support.
- No second production cloud adapter in v0.
...
- No production marketplace or hosted control plane in v0.
...
AWS is the first planned v0 adapter. Its concrete service mapping belongs in `docs/architecture/aws-adapter.md`, not in the core Cell contract.
...
- Whether the first non-AWS adapter should be a local static/runtime bundle, Fly.io, Cloudflare, or another target after v0.
- License choice before open sourcing.
