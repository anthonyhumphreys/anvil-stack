...
export async function createRuntimeContext(
  ...
  const identity =
    request.kind === "workflow" || request.kind === "service"
      ? null
      : requestHasAuth(request)
  ...
  return {
    ...
    events: host.events,
    jobs: host.jobs,
    workflows: createWorkflowClient(host.workflows),
    request: createRequestContext(request, handler),
  };
}

function createWorkflowClient(adapter: WorkflowAdapter): WorkflowClient {
  return {
    start(name, input) {
      return adapter.start(name, input);
    },
  };
}
