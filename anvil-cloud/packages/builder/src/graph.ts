...
export type AnvilCellGraph = {
  ...
  permissions: Array<{
    ...
    to: string;
    targetKind: "table" | "secret";
  }>;
};

export type GraphValidationDiagnostic = {
  code: string;
  message: string;
  path: string;
};
...
export function createAnvilCellGraph(manifest: CellManifest): AnvilCellGraph {
  ...
  const functions = new Map<
    string,
    { cell: string; name: string; runtime: string; handler: string }
  >();
  ...
  const tables = Object.keys(manifest.schema.tables).map((name) => ({
    cell: cellName,
    name,
    access: "read-write" as const,
  }));
  const secrets = readSecretNames(manifest.capabilities).map((name) => ({
    cell: cellName,
    name,
  }));
  const permissions = Array.from(functions.keys()).flatMap((functionName) => [
    ...
    ...secrets.map((secret) => ({
      ...
      to: secret.name,
      targetKind: "secret" as const,
    })),
  ]);
  return {
    schemaVersion: "0.1",
    appName: manifest.cell.name,
    cells: [{ name: cellName, runtime: manifest.cell.runtime }],
    httpRoutes,
...
export function validateAnvilCellGraph(
  ...
  for (const route of graph.httpRoutes) {
    if (!functions.has(route.handler))
      diagnostics.push({
        code: "GRAPH_ROUTE_HANDLER_MISSING",
        message: `HTTP route ${route.method} ${route.path} references missing function '${route.handler}'.`,
        path: `httpRoutes.${route.method} ${route.path}.handler`,
      });
  }
  for (const permission of graph.permissions) {
    const targets = permission.targetKind === "table" ? tables : secrets;
    if (!functions.has(permission.from))
      diagnostics.push({
        code: "GRAPH_PERMISSION_SOURCE_MISSING",
        message: `Permission source function '${permission.from}' is missing.`,
        path: `permissions.${permission.from}`,
      });
    if (!targets.has(permission.to))
      diagnostics.push({
        code: "GRAPH_PERMISSION_TARGET_MISSING",
        message: `Permission target '${permission.to}' is missing.`,
        path: `permissions.${permission.to}`,
      });
  }
...
function readSecretNames(capabilities: Record<string, unknown>): string[] {
  ...
  if (direct && typeof direct === "object")
    return Object.keys(direct as Record<string, unknown>);
