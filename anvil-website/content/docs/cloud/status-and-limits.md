...
| Local runtime | Local HTTP server, JSON database, files, auth, logs, traces, events, jobs, workflows, supervised services, manifest, and inspection state exist. |
...
| Lens | Local Lens is served at `/_anvil/lens` and reads the same local runtime truth as CLI JSON: manifest, capabilities, auth users, logs, traces, database state, workflows, services, and diagnostics. |
