...
| No hosted control-plane service | Local Lens, the inspection `ControlPlaneApi`, and the agent execution control-plane contract now exist. Execution leases can persist to JSON, replay cursor events, enforce budgets, and use a framework-neutral HTTP router/client, but authenticated hosted route wiring, workspace authorisation, source storage, and production persistence are not deployed. See [Agent Sandboxes](/docs/cloud/agent-sandboxes). |
...
| Agent Sandboxes have an execution foundation | Runtime sandbox and execution contracts, local Docker/process providers, idempotent leases, durable cursor events, controls, budgets, cleanup receipts, deterministic conformance, hosted HTTP boundary, and the AWS Lambda MicroVM read-only transport exist. Credential injection, source snapshot storage, a deployed compatible worker image, sandbox-aware Lens, and real-account remote verification are not implemented. See [Agent Sandboxes](/docs/cloud/agent-sandboxes). |
...
- hosted approval, memory, source snapshot, credential broker, and durable
  execution services for agents
