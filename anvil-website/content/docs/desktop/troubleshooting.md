...
- **Apple Foundation Models test fails**: confirm the Mac is running macOS 26 or later, supports Apple Intelligence, and has Apple Intelligence enabled. On macOS 27, either accept the fm CLI legal notice (`sudo fm license`) or keep a Swift toolchain that can import `FoundationModels`; on macOS 26 the Swift toolchain is required. The Settings status card shows which backend Anvil is using and the model's availability reason.
- **Image attachments never route on-device**: image prompts need macOS 27 plus a backend with the vision API — the `fm` CLI after `sudo fm license`, or a Swift toolchain whose FoundationModels framework exposes image attachments. Text-only routing still works on older builds.
