...
import { ChevronDown, Download, Github, Menu, UserRound } from "lucide-react";
    ...
    <header className="sticky top-0 z-40 border-b bg-background/95 supports-[backdrop-filter]:bg-background/88 supports-[backdrop-filter]:backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Main navigation">
          ...
          <Link href="/sync" aria-current={active === "sync" ? "page" : undefined} className="flex min-h-11 items-center rounded-md px-3 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground aria-[current=page]:bg-muted aria-[current=page]:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Sync</Link>
          <Link href="/pricing" aria-current={active === "pricing" ? "page" : undefined} className="flex min-h-11 items-center rounded-md px-3 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground aria-[current=page]:bg-muted aria-[current=page]:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Pricing</Link>
        ...
        <div className="hidden items-center gap-1 md:flex">
          <ThemeToggle />
          {accountEnabled ? (
            <Link href="/account" aria-current={active === "account" ? "page" : undefined} className="inline-flex min-h-11 items-center gap-2 rounded-md px-3 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground aria-[current=page]:bg-muted aria-[current=page]:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><UserRound className="size-4" aria-hidden="true" />Account</Link>
          ) : null}
        ...
        <div className="flex items-center gap-1 md:hidden">
          ...
          <details className="group relative">
            ...
            <nav className="absolute right-0 top-12 z-50 grid w-64 gap-1 rounded-lg border bg-background p-2 shadow-lg" aria-label="Mobile navigation">
              ...
              <Link href="/sync" className="flex min-h-11 items-center rounded-md px-3 text-sm font-medium hover:bg-muted">Sync</Link>
              <Link href="/pricing" className="flex min-h-11 items-center rounded-md px-3 text-sm font-medium hover:bg-muted">Pricing</Link>
              ...
              {accountEnabled ? (
                <Link href="/account" className="flex min-h-11 items-center gap-2 border-t px-3 text-sm font-medium text-muted-foreground hover:text-foreground"><UserRound className="size-4" aria-hidden="true" />Account</Link>
              ) : null}
