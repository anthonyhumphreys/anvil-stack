    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
            ...
            <div className="max-w-3xl self-end">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Button asChild variant="outline">
                  <Link href="/docs/project/repositories">Repository map</Link>
                </Button>
              </div>
            </div>
          </div>
        ...
        <section className="py-14">
          <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[16rem_1fr] lg:px-8">
            <div>
            ...
            <div className="grid gap-8 lg:grid-cols-2">
              {groupedDocs.map(({ product, sections }) => (
                <div key={product} className="rounded-lg border bg-card p-5 shadow-sm">
                  <h3 className="text-xl font-semibold tracking-normal">{product}</h3>
                      ...
                      <div key={`${product}-${section}`}>
                        ...
                        <div className="mt-2 grid gap-1">
                          {entries.map((entry) => (
                            ...
                            >
                              <span className="font-medium text-foreground">{entry.navTitle}</span>
                              {entry.description ? <span className="mt-1 block leading-6">{entry.description}</span> : null}
                            </Link>
                          ))}
                        </div>
                      </div>
